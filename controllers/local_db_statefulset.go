...
func (r *BackstageReconciler) applyLocalDbServices(ctx context.Context, backstage bs.Backstage, ns string) error {
	...
	err := r.applyPsqlService(ctx, backstage, name, name, DefaultLocalDbService, ns)
	if err != nil {
		return err
	}
	nameHL := fmt.Sprintf("backstage-psql-%s-hl", backstage.Name)
	return r.applyPsqlService(ctx, backstage, nameHL, name, DefaultLocalDbServiceHL, ns)

}

func (r *BackstageReconciler) applyPsqlService(ctx context.Context, backstage bs.Backstage, name, label, defaultData, ns string) error {
