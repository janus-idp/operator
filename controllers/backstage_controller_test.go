	...
	When("creating default CR with no spec", func() {
		...
		It("should successfully reconcile a custom resource for default Backstage", func() {
			...
			Eventually(func(g Gomega) {
				...
				err := k8sClient.Get(ctx, types.NamespacedName{Namespace: ns, Name: name}, found)
				g.Expect(err).ShouldNot(HaveOccurred())
				g.Expect(findStatefulSetDBSecretName(found)).Should(Equal(utils.GenerateRuntimeObjectName(backstage.Name, "default-dbsecret")))
				//fmt.Sprintf("backstage-psql-secret-%s", backstage.Name)))
			}, time.Minute, time.Second).Should(Succeed())
			...
			Expect(findDeploymentDBSecretName(found)).Should(Equal(utils.GenerateRuntimeObjectName(backstage.Name, "default-dbsecret")))
			//fmt.Sprintf("backstage-psql-secret-%s", backstage.Name)))
			...
			`),
					...
					It("should reconcile", func() {
						...
						By("Checking the main container Volume Mounts in the Backstage Deployment", func() {
							...
							// 10 in fact since Key feature is not supported (yet)
							...
							// since Key feature is not supported there are 2, not 1
			...
			When("setting environment variables either directly or via references to ConfigMap or Secret", func() {
				...
				It("should reconcile", func() {
					...
					By(fmt.Sprintf("Checking Env in the Backstage Deployment - container: %q", mainCont.Name), func() {
						...
						// TODO 3 by some reason: Most probably because Key is not supported
						...
						// TODO Most probably because Key is not supported
