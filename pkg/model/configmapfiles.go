...
func (f ConfigMapFilesFactory) newBackstageObject() BackstageObject {
	return &ConfigMapFiles{configMap: &corev1.ConfigMap{}}
}
...
func (p *ConfigMapFiles) updateBackstagePod(pod *backstagePod) {
	path := defaultDir
	paths := make([]string, 0)
	for k := range p.configMap.Data {
		paths = append(paths, filepath.Join(path, k))
	}
	pod.addExtraFilesFromConfigMap(p.configMap.Name, paths)
}
